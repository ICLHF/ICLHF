...
def slab(x0=None, y0=None, z0=None, x1=None, y1=None, z1=None, k=None):
    fs = []
    if x0 is not None:
        fs.append(plane(X, np.array((x0, 0, 0))))
    if x1 is not None:
        fs.append(plane(-X, np.array((x1, 0, 0))))
    if y0 is not None:
        fs.append(plane(Y, np.array((0, y0, 0))))
    if y1 is not None:
        fs.append(plane(-Y, np.array((0, y1, 0))))
    if z0 is not None:
        fs.append(plane(Z, np.array((0, 0, z0))))
    if z1 is not None:
        fs.append(plane(-Z, np.array((0, 0, z1))))
    return intersection(*fs, k=k)


@sdf3
def box(size=np.array(1), center=ORIGIN, a=None, b=None):
