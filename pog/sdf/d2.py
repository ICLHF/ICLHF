...
@sdf2
def circle(radius=1.0, center=ORIGIN):
    def f(p):
...
def slab(x0=None, y0=None, x1=None, y1=None, k=None):
    fs = []
    if x0 is not None:
        fs.append(line(X, np.array((x0, 0))))
    if x1 is not None:
        fs.append(line(-X, np.array((x1, 0))))
    if y0 is not None:
        fs.append(line(Y, np.array((0, y0))))
    if y1 is not None:
        fs.append(line(-Y, np.array((0, y1))))
    return intersection(*fs, k=k)


@sdf2
def rectangle(size=np.array(1), center=ORIGIN, a=None, b=None):
