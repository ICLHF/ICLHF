...
from pog.planning.search_node import SearchNode
from pog.planning.search_problem import Arc, Path, SearchProblem


class PlanningOnGraphArc(Arc):
    def __init__(self, from_node: SearchNode, to_node: SearchNode):
        super().__init__(from_node, to_node, to_node.cost, to_node.action)
...
class PlanningOnGraphProblem(SearchProblem):
    def __init__(self, start: Graph, goal: Graph, parking_place=None):
        self.start_graph = start
        self.goal_graph = goal
        self.parking_place = (
            self.start_graph.root if parking_place is None else parking_place
        )
        action_seq, constraints = action_seq_generator(
            self.start_graph, self.goal_graph, self.parking_place
        )
    ...
    def neighbors(self, node: SearchNode):
        ...
        for neighbor in node.selectAction():
            ...
            if (
                new_action.action_type == ActionType.Place
                or new_action.action_type == ActionType.PicknPlace
            ):
                ...
                if new_action.add_edge[1] in unstable_nodes:
                    ...
                    temp_new_action_place = Action(
                        (None, new_action.add_edge), action_type=ActionType.Place
                    )
                    new_action.add_edge = (
                        self.start_graph.edge_dict[new_action.add_edge[1]].parent_id,
                        new_action.add_edge[1],
                    )
                    new_action.reverse = True
                    temp_new_action_pick = Action(
                        (new_action.add_edge, None), action_type=ActionType.Pick
                    )
                    new_action_seq.extend([temp_new_action_pick, temp_new_action_place])
                    ...
                    for succ in current.graph.successors(new_action.add_edge[1]):
                        del_succ = Action(
                        ...
                        if del_succ in new_action_seq:
                            ...
                            constraints.addConstraint(to_ground, temp_new_action_pick)
                            constraints.addConstraint(
                                temp_new_action_place, from_ground
                            )
                            assert add_succ is not None
                ...
                if is_stable:
                    _, names = current.collision_manager.in_collision_internal(  # type: ignore
                        return_names=True
                    )
                    is_collision = False
                    collide_items = []
                    ...
                    if is_collision:
                        ...
                        new_action = Action(
                            (None, (self.parking_place, old_action.add_edge[1])),
                            action_type=ActionType.Place,
                            optimized=False,
                        )
                        old_action_pick = Action(
                            ((self.parking_place, old_action.add_edge[1]), None),
                            action_type=ActionType.Pick,
                        )
            ...
            to_node = SearchNode(
                new_action_seq, constraints, new_action, current, self.goal_graph
            )
            neighbors.append(PlanningOnGraphArc(node, to_node))
        return neighbors
