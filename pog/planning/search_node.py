...
from pog.planning.action import Action, ActionConstraints, ActionType
...
class SearchNode:
    def __init__(
        ...
        self.action = action
        if action is not None and self.action.action_type == ActionType.Pick:  # type: ignore
            self.occupied = True
        else:
            self.occupied = False
