...
class Searcher:
    ...
    def search(self):
        ...
        """
        cnt = 0
        while not self.empty_frontier() and cnt < self.MAX_TRIAL:
            cnt += 1
            ...
            if self.problem.is_goal(path.end()):  # solution found
                logging.info(
                    "{} paths have been expanded and {} paths remain in the frontier.".format(
                        self.num_expanded, len(self.frontier)
                    )
                )
        ...
        logging.info(
            "No (more) solutions. Total of {} paths expanded.".format(self.num_expanded)
        )
...
def test_search(SearchClass, problem: PlanningOnGraphProblem):
