...
class Action:
    def __init__(
        self, edge_edit_pair, action_type=None, agent="", optimized=True
    ) -> None:
    ...
    def __eq__(self, other) -> bool:
        return (
            hasattr(other, "action_type")
            and hasattr(other, "agent")
            and hasattr(other, "del_edge")
            and hasattr(other, "add_edge")
            and self.action_type == other.action_type
            and self.agent == other.agent
            and self.del_edge == other.del_edge
            and self.add_edge == other.add_edge
        )
    ...
    def breakAction(self):
        assert (
            self.del_edge is not None and self.add_edge is not None
        )  # Can only break full actions
        assert self.action_type == ActionType.PicknPlace
        return (
            Action((self.del_edge, None), action_type=ActionType.Pick, optimized=False),
            Action(
                (None, self.add_edge), action_type=ActionType.Place, optimized=False
            ),
        )
...
def updateGraph(current: Graph, goal: Graph, action_seq: List[Action], optimize=False):
    for action in action_seq:
        if action is None:
            ...
            if action.optimized:
                current.addNode(
                    action.add_edge[0], edge=goal.edge_dict[action.add_edge[1]]
                )
            else:
                ...
                if current.root == action.add_edge[0]:
                    ...
                    parent_aff_name = PairedSurface[
                        current.edge_dict[action.add_edge[0]]
                        .relations[AffordanceType.Support]["child"]
                        .name
                    ]

                child_aff_name = (
                    goal.edge_dict[action.add_edge[1]]
                    .relations[AffordanceType.Support]["child"]
                    .name
                )
                edge.add_relation(
                    current.node_dict[action.add_edge[0]].affordance[parent_aff_name],
                    current.node_dict[action.add_edge[1]].affordance[child_aff_name],
                    dof_type="x-y",
                    pose=[0, 0, 0],
                )
                ...
                if optimize:
                    ...
                    while not cnt_sat and idx < MAX_UPDATE_ITER:
                        ...
                        _, cnt_sat, _ = simulated_annealing_structure(
                            current, fixed_nodes=fixed_nodes, random_start=True
                        )

        elif action.action_type == ActionType.Open:
            current.node_dict[action.del_edge[0]].State = ContainmentState.Opened
        elif action.action_type == ActionType.Close:
            current.node_dict[action.del_edge[0]].State = ContainmentState.Closed
            ...
            if action.optimized:
                ...
                child_aff_name = (
                    current.edge_dict[action.add_edge[1]]
                    .relations[AffordanceType.Support]["child"]
                    .name
                )
                edge.add_relation(
...
def generate_seq_from_accessibility(
    g1: Graph, g2: Graph, action_seq, constraints: ActionConstraints
):
        ...
        if action.action_type == ActionType.Pick:
            ...
            if not g1.node_dict[action.del_edge[1]].accessible:
                path_del = nx.shortest_path(
                    g1.graph, source=g1.root, target=action.del_edge[1]
                )
                for idx in range(len(path_del) - 1):
                    if (
                        g1.node_dict[path_del[idx]].state == ContainmentState.Closed
                        and g1.edge_dict[path_del[idx + 1]].containment
                    ):
                        open_action = Action(
                            (
                ...
                for idx in range(len(temp_open_seq)):
                    if idx < len(temp_open_seq) - 1:
                        constraints.addConstraint(
                            temp_open_seq[idx], temp_open_seq[idx + 1]
                        )
                        constraints.addConstraint(
                            temp_close_seq[idx + 1], temp_close_seq[idx]
                        )
            ...
            if not g2.node_dict[action.add_edge[1]].accessible:
                path_add = nx.shortest_path(
                    g2.graph, source=g2.root, target=action.add_edge[1]
                )
                for idx in range(len(path_add) - 1):
                    if (
                        g2.node_dict[path_add[idx]].state == ContainmentState.Closed
                        and g2.edge_dict[path_add[idx + 1]].containment
                    ):
                        open_action = Action(
                            (
